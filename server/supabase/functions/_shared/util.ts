...
export async function checkRateLimit(
  ...
  if (!identifier.value) {
    console.error(
      `Rate limit check for action '${actionType}' failed: No identifier.`,
    );
  ...
  if (configError || !config || config.length < 2) {
    console.error(
      `Rate limit config missing in 'app_config' for action: ${actionType}`,
    );
    return { error: errorResponse(500, "Server configuration error.") };
  }

  const limitCountStr = config.find((c) =>
    c.key === `${actionType}_limit_count`
  )?.value;
  const limitMinutesStr = config.find((c) =>
    c.key === `${actionType}_limit_minutes`
  )?.value;

  // Add a check to ensure we actually found the values before parsing.
  if (!limitCountStr || !limitMinutesStr) {
    console.error(
      `Incomplete rate limit config in 'app_config' for action: ${actionType}`,
    );
    return { error: errorResponse(500, "Server configuration error.") };
  }
  ...
  const timeWindowStart = new Date(Date.now() - limitMinutes * 60 * 1000)
    .toISOString();
  ...
  if (count !== null && count >= limitCount) {
    const message =
      `Too Many Requests. Limit is ${limitCount} requests per ${limitMinutes} minutes.`;
